...
def cidadesJson(request):
    ...
    if resposta_cidades.status_code == 200:
        ...
        if dados_cidades and 'data' in dados_cidades:
    ...
    dados_previsao = resposta_previsao.json() if resposta_previsao.status_code == 200 else None
    dados_classes = resposta_classes.json() if resposta_classes.status_code == 200 else None

    contexto = {}

    if dados_previsao and 'data' in dados_previsao:
        nome_cidade = cidades[dados_previsao['globalIdLocal']]

        previsao_hoje = dados_previsao['data'][0]
        id_weather_type = previsao_hoje.get('idWeatherType')
        ...
        if id_weather_type and dados_classes:
            for dado in dados_classes['data']:
                if id_weather_type == dado.get('idWeatherType'):
        ...
        for previsao in dados_previsao['data'][1:5]:
            id_weather_type = previsao.get('idWeatherType')
            padded_number = str(id_weather_type).zfill(2) if id_weather_type else None
            icon_url = f"/static/meteo/w_ic_d_{padded_number}anim.svg" if id_weather_type else None
